...
func init() {
	...
	setLogger(&DelayQueueLogger{defaultLogger.Sugar()})
}

// InitLogger is init global logger for delayQueue
func InitLogger(config Config) (err error) {
	l, err := initDelayQueueLogger(config)
	if err != nil {
		return err
	}
	setLogger(l)
...
// initDelayQueueLogger is init delayQueue default logger
func initDelayQueueLogger(config Config) (Logger, error) {
	if config.CustomLogger != nil {
		return &DelayQueueLogger{config.CustomLogger}, nil
	}
	...
	return &DelayQueueLogger{zaplogger.Sugar()}, nil
}
